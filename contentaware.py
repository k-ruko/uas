# -*- coding: utf-8 -*-
"""
Created on Wed Nov  6 08:14:26 2024
...
image= [cv2.imread("F:/semester5/citra digital/tugas 5/gambar5.png")]
LARGE_VAL = float('Inf')
...
width_reduction = 90 / 100
rotate = not int(1)

image =cv2.imread("C:/Users/k-ruko/Downloads/WhatsApp Image 2024-11-04 at 22.55.27.jpeg")
print ("Image shape")
print (image.shape)
...
for iii in range(1, image_shape[0]):
    for jjj in range(1,image_shape[1]+2):
...
for cols_removed in range(0, number_of_cols_to_remove):
    ...
    try:
        ...
        for iii in range(new_shape[0]-1, 0, -1):
...
print ("gradient_energy"
       , gradient_energy)
print ("gradient_energy_x"
       , gradient_energy_x)
print ("gradient_energy_y"
       , gradient_energy_y)
print (image.shape)

cv2.imshow("Input", image)
cv2.imshow("Output", image_seam_reduced)
cv2.imshow ("gradient_energy", gradient_energy)
cv2.imshow ("gradient_energy_x", gradient_energy_x)
cv2.imshow ("gradient_energy_y", gradient_energy_y)
cv2.waitKey()
